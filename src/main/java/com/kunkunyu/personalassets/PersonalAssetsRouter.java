...
@Component
@AllArgsConstructor
public class PersonalAssetsRouter {

    private static final String GROUP_PARAM = "group";

    private static final int DEFAULT_PAGE_SIZE = 20;

    private PersonalAssetsFinder personalassets;
     ...
     */
    @Bean
    RouterFunction<ServerResponse> personalAssetRouter() {
    ...
    private HandlerFunction<ServerResponse> handlerFunction() {
        return request -> templateNameResolver.resolveTemplateNameOrDefault(request.exchange(), "personalassets")
            .flatMap(templateName -> ServerResponse.ok().render(templateName,
                Map.of("groups", personalAssetGroups(),
                    "personalAssets", personalAssetList(request),
    ...
    private Mono<UrlContextListResult<PersonalAssetVo>> personalAssetList(ServerRequest request) {
        String path = request.path();
        int pageNum = pageNumInPathVariable(request);
        String group = groupPathQueryParam(request);
        return personalassets.list(pageNum, DEFAULT_PAGE_SIZE, group)
            .map(list -> new UrlContextListResult.Builder<PersonalAssetVo>()
                .listResult(list)
    ...
    private Mono<List<PersonalAssetGroupVo>> personalAssetGroups() {
